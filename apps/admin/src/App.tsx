 ...
 *
 * «Промпты» — единственный раздел, который может быть **выключен**: без
 * отчётов прогонов набора судить о §10.3 не по чему (рантбук, «Раздел
 * промптов: почему его может не быть»). Вкладка при этом остаётся, но
 * пустой не бывает: раздел печатает причину и команду, которой это
 * лечится. Прежде он печатал в этом состоянии «Не удалось прочитать
 * промпты» — то есть звал искать поломку, которой нет.
 */
const TABS = [
