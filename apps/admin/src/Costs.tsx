...
/**
 * Что печатается вместо суммы, когда её нет вовсе.
 *
 ...
 * неизвестно, понять нельзя. Состояние штатное, а не выдуманное:
 * сорвавшиеся вызовы удалившего данные человека дают и пустого человека,
 * и пустую цену разом. Пустая клетка отчёта читается как ноль, то есть
 * как факт, — поэтому здесь сказано словами.
 */
const NO_SUM =
  ...
  return (
    <div data-testid="costs">
      ...
      {/*
        ...
        Отсюда и две ветки текста: вызовы есть, а суммы у них может не
        быть вовсе — тогда прочерк вместо суммы врал бы про ноль (см.
        NO_SUM). Утверждение «сумма по людям меньше общей ровно на эту
        величину» стоит только в ветке с суммой: без цены она общей и не
        меньше — неизвестное не вычитается.
      */}
      {report.unattributedCalls > 0 && (
        <p className="оговорка" data-testid="unattributed">
          {report.unattributed.length === 0
            ? `Ещё ${callsWord(report.unattributedCalls)} к моделям от тех, кто удалил свои данные, — ${NO_SUM}.`
            : `Ещё ${money(report.unattributed)} потрачено на тех, кто удалил свои данные — ${callsWord(report.unattributedCalls)} к моделям. Сумма по людям меньше общей ровно на эту величину.`}{' '}
      ...
      {report.unlinkedCalls > 0 && (
        <p className="оговорка" data-testid="unlinked">
          {report.unlinked.length === 0
            ? `Ещё ${callsWord(report.unlinkedCalls)} вне выгрузок — ${NO_SUM}.`
            : `Ещё ${money(report.unlinked)} потрачено вне выгрузок — ${callsWord(report.unlinkedCalls)}.`}{' '}
