...
export interface SettingRow {
  ...
  /**
   * Что лежит в базе, если пределы это значение отвергли.
   *
   * Пусто в обычном случае. Непусто означает разлад: в строке одно,
   * работает другое — и панель обязана сказать это словами. Прежде она
   * печатала действующее число без пометки «(из кода)», то есть
   * утверждала, что в базе именно оно.
   */
...
export interface SettingsPage {
  ...
  /**
   * Настройки, у которых нет читателя в коде, — словами.
   *
   * Подпись была шире: «чего §15 просит, а здесь пока нет». Сервер
   * кладёт в поле не это, а список настроек без вызывающего, и разбор
   * ревизии поймал подпись на слове: пустой список читался как «§15
   * закрыт целиком», хотя означает лишь «у каждой настройки есть
   * читатель».
   */
  readonly missing: readonly string[];
}
