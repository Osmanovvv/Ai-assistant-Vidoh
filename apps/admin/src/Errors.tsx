    ...
    <div data-testid="errors">
      ...
      <div className="разрез">
        <h3 className="разрез__имя">
          ...
          {page.callsTotal > 0 && `— всего ${String(page.callsTotal)}`}
        </h3>

        {/*
          Отправка, а не вызов (§10.5 ТЗ).

          Учёт стоит внутри повтора, и каждая сорвавшаяся отправка видна
          отдельной строкой — именно этого ТЗ и просит. Но читать её как
          «человек остался без разбора» нельзя: повтор чаще всего удаётся. Кого
          действительно подвело — в верхней таблице, у сорвавшихся разборов.
        */}
        <p className="оговорка">
          Строка — одна отправка, а не весь вызов: сорвавшаяся попадает сюда и тогда, когда повтор
          удался и человек разбор получил. Кого подвело на самом деле — выше, у сорвавшихся
          разборов.
        </p>

        {page.calls.length === 0 ? (
          <p className="разрез__пусто">Неуспешных отправок за этот срок нет.</p>
        ) : (
          <div className="таблица-обёртка">
            <table className="таблица">
              <thead>
