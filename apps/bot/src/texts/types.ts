...
export interface TextProfile {
  ...
  readonly resolver: {
    ...
    /**
     * Распоряжение о записи, цель которого не нашлась (задача 3.67).
     *
     * «Перенеси дело с собакой на вторник» при трёх делах про собаку.
     * Прежде такое распоряжение уходило в разбор и становилось **делом**
     * с этим же текстом. Теперь оно сохраняется как есть (§16) и человеку
     * говорится честно, что цель не найдена.
     */
    readonly targetNotFound: string;
  };
  /**
  ...
  readonly backlog: {
    ...
    /**
     * Посмотреть не вышло: вектор вопроса не посчитался.
     *
     * Отдельная реплика, а не «ничего не записано»: разница для человека
     * решающая. Отсутствие он примет за факт и не переспросит, а про наш
     * сбой переспросит — и запись найдётся.
     */
  ...
  readonly reminders: {
     ...
     *
     * Утреннее и вечернее напоминания каждый день приглашали выгружать
     * того, кому бот в ответ откажет: «наговори, разложу» — а на
     * наговорённое приходит «пробные разборы закончились». Приглашение,
     * которое бот сам не исполнит, хуже молчания: оно повторяется
     * ежедневно.
     */
  ...
  readonly onboarding: {
    ...
    /**
     * Вопрос про имя, когда имя из профиля в него не годится — пусто,
     * точка, одни знаки (правка заказчика 04.09.2026).
     *
     * Раньше такого человека не спрашивали вовсе: показывать ему
     * «Называть тебя .?» значило показать сбой. Спрашиваем прямо, не
     * поминая его непригодное имя.
     */
    readonly nameUnknown: string;
    /** Отказ там, где подтверждать нечего: «поправлю потом» — не про это. */
    readonly buttonNameSkip: string;
  ...
  readonly billing: {
    ...
    /**
     * Оплаченный период кончился (ревизия четвёртого этапа).
     *
     * Отдельно от «пробные разборы закончились»: человек, который платил,
     * не касался пробного периода, и та реплика читалась им как «бот
     * забыл мою оплату».
     */
    readonly paidOver: string;
    /** Продление не прошло, период кончился: карта или деньги. */
    readonly renewalOver: string;
     ...
     * п. 2.3 и 7.2; требование Робокассы к форме оплаты).
     *
     * Сумма, периодичность и дата первого списания — до оплаты, а не
     * после: человек, узнавший о списании из банка, перестаёт доверять
     * продукту. Отдельно сказано, что без отметки платёж разовый: отметка
     * не проставлена заранее, и это должно быть понятно, а не выглядеть
     * забытой галочкой.
     */
    readonly consentScreen: (price: string, firstCharge: string) => string;
    /** Строка согласия в тексте экрана: пустой квадрат и отмеченный. */
    readonly consentOff: string;
    readonly consentOn: string;
    /** Кнопка-переключатель согласия: короче строки, чтобы уместиться. */
    readonly buttonConsentOff: string;
