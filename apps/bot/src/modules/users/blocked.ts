 ...
 *
 * **Пока не вызывается, и это осознанно.** Аудит 27.08.2026: отправки
 * статусов и вопросов не повторяются вовсе — ошибка глотается, человек
 * помечается, конвейер идёт дальше. Значит условие «повторных попыток
 * нет» выполнено без этой функции.
 *
 * Оставлена не «на всякий случай», а под напоминания по расписанию
 * (этап 3): вот там отправка будет повторяться, и там же понадобится
 * знание, что 400 — это наша ошибка в запросе, а не беда получателя.
 * Знание добыто дорого: «chat not found» приходит с кодом 400, и
 * проверка на 403 не срабатывала никогда.
 */
