 ...
 *
 * **Эмоции сюда не попадают (задача 3.48).** §13.7 говорит прямо:
 * «эмоция распознаётся как отдельный тип, но **не создаёт записи**». У
 * нас она всё-таки хранится — из неё бот понимает состояние, и она
 * входит в выгрузку данных по §16, — но человеку показывать её как
 * пункт списка нельзя.
 *
 * Найдено 03.09.2026 на живой сводке: в ветке «личное» строкой висело
 * «Я уже задолбался всё это в голове держать, такое ощущение, что ничего
 * не успеваю» — наравне с «позвонить в банк». Человек пожаловался
 * вслух, а продукт вернул ему жалобу пунктом дел.
 *
 * Желания, замыслы и сведения остаются: их человек записывал сознательно
 * и хочет видеть. §6.3 запрещает делать из них **задачи**, а не
 * показывать их вовсе.
 *
 * Фильтр стоит в запросе, а не в сборке текста: тем же запросом живёт
 * список сферы в меню (§12.1), и там правило то же.
 */
