...
/**
 * Закончить рассылку — и разобраться с просьбой остановиться.
 *
 * **Законченная рассылка с непустой меткой жила навсегда** (ревизия
 * панели). Метку ставит панель, снимают только `retryFailed` и
 * `resumeBroadcast`; `settleStopRequests` лечит одни `running`. А повтор
 * неудачных требует пустой метки — значит у такой рассылки кнопка
 * «Повторить неудачные» в панели была, а сервер отвечал 409 на каждое
 * нажатие. Достижимо это двумя путями: просьба успевала прийти между
 * последней отметкой и `finishBroadcast(..., 'done')` (broadcast.service),
 * и задание исчерпывало три попытки после просьбы (index.ts).
 *
 * Поэтому метка разбирается по состоянию, а не снимается всегда:
 *
 *  - `done` — слать больше нечего: заход закрывает рассылку только по
 *    пустым `pending` и `sending`. Просьба исполнилась сама собой, и
 *    держать её значит запрещать повтор **неудачных** писем — тех, что
 *    никому не дошли;
 *  - `failed` — неотправленные строки могли остаться, и просьба ещё в
 *    силе. Такая рассылка объявляется **остановленной**: то же решение
 *    принимает `settleStopRequests` при старте бота. Иначе повтор
 *    вернул бы в очередь всех оставшихся, а человек просил остановиться
 *    — это и есть то самое «письмо ушло восьмистам»;
 *  - `stopped` — метка остаётся. Сними её, и зависшее задание,
 *    проснувшись, просьбы уже не увидит и продолжит отправку.
 *
 * Решение принимается **в самом запросе**: между «прочитали метку» и
 * «записали статус» помещается нажатие «Продолжить» в панели.
 */
export async function finishBroadcast(
  db: Executor,
  id: string,
  status: 'stopped' | 'done' | 'failed',
): Promise<'stopped' | 'done' | 'failed' | undefined> {
  const [row] = await db
    .update(broadcasts)
    .set({
      status:
        status === 'failed'
          ? sql`case when ${broadcasts.stopRequestedAt} is null then 'failed' else 'stopped' end`
          : status,
      finishedAt: new Date(),
      ...(status === 'done' ? { stopRequestedAt: null } : {}),
    })
    .where(and(eq(broadcasts.id, id), ne(broadcasts.status, status)))
    .returning({ status: broadcasts.status });

  /**
   * Чем всё кончилось — возвращаем, а не догадываемся.
   *
   * Запрошенный статус и полученный расходятся нарочно: «попытки
   * исчерпаны» у рассылки с просьбой остановиться означает
   * «остановлена», а не «сорвалась». Без ответа вызывающий писал в
   * журнал «объявлена сорвавшейся» в обоих случаях — то есть журнал
   * врал ровно в том месте, куда идут разбираться с рассылкой.
   *
   * Пусто означает «ничего не изменилось»: строка уже была в этом
   * состоянии (условие `ne` выше) или её нет вовсе.
   */
  const became = row?.status;

  if (became === undefined) return undefined;

  /**
   * Сужение здесь, а не у вызывающего.
   *
   * Столбец состояния — обычный текст, а писатель у него один: этот
   * файл. Незнакомое слово означало бы, что в столбец пишут мимо
   * репозитория, и промолчать об этом хуже, чем упасть: журнал сбоев —
   * последнее место, где уместна догадка.
   */
  if (became !== 'stopped' && became !== 'done' && became !== 'failed') {
    throw new Error(`рассылка ${id} закончилась неизвестным состоянием: «${became}»`);
  }
...
/**
 * Сколько рассылок всего — рядом со списком из двадцати.
 *
 ...
 * вместе с единственным путём к «Повторить неудачные» и вместе с ответом
 * на вопрос «а это письмо мы уже отправляли?». У соседнего журнала
 * сбоев итог за период появился по той же причине.
 */
export async function countBroadcasts(db: Executor): Promise<number> {
  const [row] = await db.select({ total: count() }).from(broadcasts);
...
/**
 * Отменить черновик — единственный выход из случайно составленного.
 *
 * **Дефект, который это закрывает.** «Составить» заводит и рассылку, и
 * до тысячи строк доставки (см. `createBroadcast` выше), а кнопка
 * «Отправить» жила только в памяти браузера: уход в другой раздел или
 * обновление страницы стирали её, и у строки со статусом «черновик» не
 * оставалось ни одного действия. Отменить его было нечем вовсе — то
 * есть единственным выходом из ошибочного черновика была **отправка
 * всем**.
 *
 * Условие «только черновик» живёт **в запросе**, а не в проверке перед
 * ним: между «прочитали статус» и «удалили» помещается нажатие
 * «Отправить» в другой вкладке, и тогда мы удалили бы идущую рассылку
 * вместе с её историей. Ответ `false` означает «это уже не черновик», и
 * панель говорит об этом словами.
 *
 * Строки доставки уходят сами: у `broadcast_deliveries.broadcast_id`
 * стоит `on delete cascade` (schema.ts). Удалять их отдельно значило бы
 * держать вторую правду о том же связывании.
 */
export async function cancelDraft(db: Executor, id: string): Promise<boolean> {
  const gone = await db
    .delete(broadcasts)
    .where(and(eq(broadcasts.id, id), eq(broadcasts.status, 'draft')))
    .returning({ id: broadcasts.id });
