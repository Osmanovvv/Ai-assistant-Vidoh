...
describe('имя из слов человека', () => {
  ...
  it('без букв — тоже имя, если человек так решил', () => {
    /**
     * Правка заказчика 04.09.2026: «вдруг человек хочет, чтобы его
     * называли „,“». Проверка на букву стояла против мусора **из профиля
     * Telegram**, где имя приходит само. Здесь человек нажал кнопку и
     * написал это сам, и решать за него не наше дело.
     *
     * Мысль от этого потерять нельзя: у мысли буквы есть всегда.
     */
  ...
  it('пустое имя не берётся', () => {
    for (const input of ['', '   ', '\n']) {
      expect(parseName(input), JSON.stringify(input)).toBeUndefined();
    }
  });
});
