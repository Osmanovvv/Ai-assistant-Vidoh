...
export const SETTINGS = {
  ...
  silenceWindowMs: {
     ...
     *
     * Не больше потолка возраста выгрузки — и это не наш выбор, а
     * граница, за которой настройка перестаёт действовать (ревизия
     * этапов 1–2, дефект 15). Прежде здесь стояли свои десять минут,
     * вдвое шире потолка: панель принимала семь минут с «Сохранено», а
     * досмотр закрывал выгрузку по возрасту на пятой — раньше, чем
     * задание дожидалось тишины. Окно от пяти до десяти минут молча
     * вырождалось в пятиминутное. Один потолок, одно число — как у
     * цены в звёздах, где реестр берёт предел у рельса.
     *
     * Отказ на записи говорит словами: «допустимо от 1000 до 300000».
     */
    min: 1_000,
    max: MAX_BATCH_AGE_MS,
    readBy: 'приём сообщений, закрытие выгрузки, досмотр и подъём процесса',
  },
