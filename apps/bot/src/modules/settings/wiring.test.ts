...
describe('предел окна тишины не обещает больше, чем держит потолок выгрузки', () => {
  ...
  it('связь одним числом, а не совпадением двух', async () => {
    ...
    const buffer = stripped(await readFile('src/modules/buffer/buffer.service.ts', 'utf8'));
    const repo = stripped(await readFile('src/modules/settings/settings.repo.ts', 'utf8'));
