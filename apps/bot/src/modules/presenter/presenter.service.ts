...
export function buildReply(params: BuildReplyParams): Reply {
  ...
  if (actions.length === 0) {
    /**
     * Дел в выдаче нет — значит и за её пределами нет: фильтр показывает
     * всё, что годится, пока не упрётся в предел, и «ноль показано при
     * скрытых» не бывает. Строка «Срочного на сегодня нет — всё
     * остальное сохранила» стояла на этот несуществующий случай и не
     ...
     */
    lines.push('', answer.nothingHidden);
