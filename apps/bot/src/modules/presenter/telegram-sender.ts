import type { InlineKeyboard, Api } from 'grammy';
...
export function createQuestionSender(deps: TelegramSenderDeps): QuestionSender {
  return {
    async ask({ chatId, threadId, text, rows }) {
      // Заданные строки можно разбить, но не слить: см. `packRows`.
      const keyboard = fitKeyboard(rows);
 ...
 *
 * **Здесь стояло «кнопки §13.2 идут одной строкой: подписи короткие».**
 * На телефоне подписи не короткие: «Оставить на потом» показывалось как
 * «Остави…потом». Раскладку считает `packButtons` по длине подписей —
 * подробности и числа там.
 */
function markup(buttons: readonly StatusButton[] | undefined): {
  ...
  return { reply_markup: fitKeyboard([buttons]) };
}
