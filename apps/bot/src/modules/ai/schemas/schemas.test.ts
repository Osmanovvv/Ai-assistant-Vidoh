...
describe('схема маршрутизатора', () => {
  ...
  it('отвергает неправдоподобное дробление', () => {
     ...
     *
     * **`crisis` здесь обязателен, и это не мелочь.** Без него объект
     * отвергался по отсутствию признака кризиса — тем же отказом, что
     * проверяет строка «нет признака кризиса» в таблице выше. Предел
     * дробления при этом не проверялся вовсе: его можно было поднять до
     * тысячи, и весь набор оставался зелёным. Теперь единственная причина
     * отказа здесь — длина списка.
     */
    const segments = Array.from({ length: 21 }, () => ({ intent: 'DUMP', text: 'дело' }));

    expect(routerSchema.safeParse({ crisis: false, segments }).success).toBe(false);
  });

  it('двадцать сегментов ещё проходят: отвергается перебор, а не сама длина', () => {
    // Обратная сторона того же стража: если он покраснеет на разрешённом
    // числе, значит предел уехал вниз и живая выгрузка из десятка мыслей
    // начнёт отваливаться целиком.
