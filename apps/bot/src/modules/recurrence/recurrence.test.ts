...
/**
 * Якорь при отвергнутом сроке — вторая находка 10.09.2026.
 *
 * Починка того же дня велела брать якорь из **проверенного** срока. Но
 * когда проверенного срока нет, якорем остаётся строка модели — как
 * было. Разведка предлагала выбрасывать правило вместе с отвергнутым
 * сроком; скептик показал, что это потеряет `weekdays` у «по будням».
 *
 * Здесь записан сам механизм — на чистой функции, без базы и без
 * модели: правило действительно собирается из непроверенной строки, и
 * действительно рассыпается, если строку не давать. Ровно это и есть
 * цена предложенной строгости, и её надо мерить, а не угадывать.
 */
describe('якорь правила при отвергнутом сроке', () => {
  const weekdays = {
  ...
  it('правило собирается из строки модели, хотя срок эту строку не прошёл', () => {
    // Ровно то, что случается с «по будням»: `resolveDeadline` отверг
    // «2026-09-07», а сюда та же строка приходит нетронутой.
    const resolved = resolveRecurrence({ ...weekdays, deadline: '2026-09-07' });
  ...
  it('без якоря правила нет — остаётся фраза человека', () => {
    // Это и произошло бы, выброси мы строку модели вместе с отвергнутым
    // сроком: `weekdays` у «по будням» превращается в фразу без правила,
    // то есть в `unclear` по мерке контрольного набора.
    const resolved = resolveRecurrence({ ...weekdays, deadline: '' });
  ...
  it('починка первой находки эту цену снимает: со сроком правило целое', () => {
    // Начни `hasTimeWord` видеть «по будням» — срок пройдёт проверку, и
    // якорь придёт из него. Тогда строгость к отвергнутому сроку этой
    // записи уже ничего не стоит. Две находки связаны именно так.
    const resolved = resolveRecurrence({ ...weekdays, deadline: '2026-09-07' });
