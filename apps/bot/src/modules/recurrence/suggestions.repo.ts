...
export async function recordOffer(
  ...
  const [row] = await db
    ...
    .values({
      ...
      /**
       * Время предложения — переданное, а не часы базы.
       *
       * Недельный предел считается именно от него, и без этой строки его
       * нельзя было проверить управляемыми часами: `now` принимался и не
       * использовался, а `created_at` брался из `now()` базы. Тест на
       * предел зависел от настоящего календаря и падал через сутки после
       * написания. В бою разницы нет — там переданное время и есть
       * настоящее, — но проверяемость важнее совпадения.
       */
