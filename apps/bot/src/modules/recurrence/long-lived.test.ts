...
describe('месячные и годовые идут по одному, но от якоря', () => {
  it('31 января ежемесячно через десять лет — по-прежнему 31-е, где месяц позволяет', () => {
    /**
     * Прежде этот страж закреплял обратное — «съезжает на 28-е и там
     * остаётся» — как поведение, которое перескок не вправе менять. Ревизия
     * этапов 1–2 показала, что это был дефект, а не поведение: шаг от уже
     * зажатого 28-го давал 28 марта, и 31-е не возвращалось никогда. Теперь
     * месячный шаг считается от якоря, и за сто двадцать два шага зажим не
     * накапливается: короткий месяц зажимает только себя.
     */
    const at = nextOccurrence(
      { kind: 'monthly', interval: 1, anchor: '2026-01-31' },
      { after: new Date('2036-03-15T09:00:00.000Z'), timeZone: MOSCOW },
    );

    expect(localDate(at, MOSCOW)).toBe('2036-03-31');
  });
