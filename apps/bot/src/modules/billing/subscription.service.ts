...
import { periodEndAfter, RENEWAL_CHARGED_BY, renewFrom, type Rail } from './tariffs.js';
...
/**
 * `hasPaidAccess` убрана ревизией этапов 1–2.
 *
 * Ревизия четвёртого этапа велела «связать с вызывающим или удалить», а
 * сделано было третье: функцию переписали на `paysNowSql` и оставили
 * без вызывающего, объяснив это тем, что «платит ли» спрашивают и вне
 * SQL — «реплики, напоминания, кнопочные ответы». Названы были три
 * места, и все три спрашивают `accessOf`: `incoming.ts` и
 * `question.ts` — реплики, `scheduler.service.ts` — напоминания,
 * `mayParseDump` ниже — кнопочный путь. То есть довод оставить был
 * неверен уже в тот день, когда его написали.
 *
 * Голого «да/нет» никому и не хватает: у отказа спрашивают причину
 * (`why`), остаток пробного и срок оплаченного — это `accessOf`. Внутри
 * запросов правду говорит `paysNowSql`, одна на всех.
 */
 ...
 *    принимаемое внутри `markTrialSpent`. Убрана ревизией этапов 1–2:
 *    вызывающего она так и не нашла.
 *
...
async function applyInside(
  ...
  /**
   * Срок считается от конца оплаченного, пока он не наступил, и от
   * «сейчас», если подписка уже закончилась (решение заказчицы,
   * 12.09.2026: заплатила позже — получает полный период с оплаты, а не
   * укороченный на дни просрочки). Разбор — у `renewFrom`.
   *
 ...
 *
 * **Отказом считается только то, что провайдер может списать без нас.**
 * У Робокассы дочернее списание уходит от нас по этой же строке, ключа
 * отмены у неё нет и не бывает — и прежде пустой ключ читался как отказ.
 * Робокассный подписчик после «удалить безвозвратно» получал реплику про
 * звёзды и обещание списаний, которых не будет (дефект №20 ревизии):
 * тревога вместо правды в самый неподходящий момент.
 */
export async function stopAllRenewals(
  ...
  for (const subscription of await subscriptionsOf(db, params.userId)) {
    ...
    if (RENEWAL_CHARGED_BY[rail] === 'us') {
      /**
       * Списываем мы — наша отметка и есть отмена.
       *
       * Провайдера не спрашиваем и не требуем: его может не быть (рельс
       * выключен), а списание без нас всё равно не уйдёт — проход
       * продления читает эту строку, и после удаления её не станет.
       * Отметка нужна на случай, если удаление сорвётся: тогда
       * состояние сойдётся с тем, что человеку сказано.
       */
    ...
    if (provider === undefined || subscription.subscriptionRef === null) {
       ...
       *
       * Рельс выключен или ключа не было, а списывает провайдер:
