...
export function createRobokassaProvider(deps: RobokassaDeps): PaymentProvider {
  ...
  return {
    ...
    async createCheckout(params: CheckoutParams): Promise<Checkout> {
      ...
      /**
       * Просим ли продление — и «просим» здесь значит именно это.
       *
       * Три условия, и третье было забыто до ревизии четвёртого этапа:
       * месячный тариф, не промо-счёт **и согласованная услуга**.
       *
       * Без третьего `Recurring=true` уходил на несогласованный магазин
       * — а несогласован он по умолчанию, — и Робокасса отвечала кодом
       * 34 **после** нажатия кнопки. То есть купить месяц было нельзя
       * вовсе, а годовой тариф работал: отказ выглядел бы как «у меня
       * почему-то не проходит оплата», и искать его пришлось бы в
       * переписке с провайдером.
       *
       * Обидно, что опасность была записана прямо здесь, в комментарии
       * ниже: «на несогласованном магазине `Recurring=true` падает кодом
       * 34». Комментарий предупреждал, а код всё равно посылал.
       */
      const wantsRenewal =
        params.plan === 'monthly' && params.renewable !== false && deps.recurringApproved === true;
      ...
      const query = new URLSearchParams({
        ...
        ...userParams,
        /**
         * Автопродление просим у месячного тарифа, не промо-счёта и
         * согласованной услуги — см. `wantsRenewal` выше.
         *
         ...
         *
         * Отказ просить приходит и от промокода (задача 4.4): дочернее
         ...
         */
        /**
         * Вместе с просьбой о продлении — только банковская карта
         * (решение заказчицы 12.09.2026, ответ 12): дочерние списания идут
         * по сохранённой карте, и страница со СБП или кошельком дала бы
         * оплату без продления, о котором бот только что пообещал. У
         * годового и у месяца без продления способ не навязывается — СБП
         * там остаётся, пока Робокасса не скажет иначе. `IncCurrLabel`
         * в подпись не входит (интерфейс оплаты, docs.robokassa.ru).
         */
        ...(wantsRenewal ? { Recurring: 'true', IncCurrLabel: 'BankCard' } : {}),
      ...
      return {
        ...
        /**
         * Обещаем ровно то, о чём попросили.
         *
         * Одной переменной, а не своим условием: обещание человеку и
         * содержимое запроса обязаны сходиться. Разойдись они — и бот
         * либо обещает продление, которого не просил, либо просит
         * продление, о котором молчит.
         */
        autoRenews: wantsRenewal,
      };
    },
