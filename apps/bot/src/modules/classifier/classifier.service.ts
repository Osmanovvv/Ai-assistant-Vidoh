 ...
 * прогон проджекта 04.09.2026 показал, чем это кончается: «хочу начать
 * делать небольшой сайт для себя, пока ничего толком не сделано» модель
 * относит к **желанию** — и совершенно верно, он так и сказал. Желание
 * проектом быть не могло, признак снимался принудительно. Проектом же
 * стал его собственный шаг «определиться со структурой сайта»: роли
 * перевернулись, шагов в базе ноль, и на вопрос «какой следующий шаг?»
 * отвечать было нечем.
 *
 * **Решение заказчика: разрешить.** «Хочу свой сайт», «хочу заняться
 * информационной безопасностью» — это цели с шагами, и люди говорят о
 * них именно так.
 *
 * **Замысел, идея и чувство остаются вне.** «Есть идея когда-нибудь
 * сделать телеграм-канал, никаких действий и сроков по нему не ставь» —
 * человек прямо запретил шаги, а проект без шагов не проект.
 *
 * **И разрешение сделано не наполовину.** Напоминания «неделя без
 * движения» остались только у задач (`scheduler.service.ts`): ценность
 * признака у желания — ответить, когда человек **спросил**, а не окликать
 * его самому. Желание — это список «когда-нибудь», и давить им нельзя
 * (§6.3).
 */
...
export function correctItems(
  ...
  for (const [index, item] of raw.items.entries()) {
    ...
    /**
     * §5.1 ТЗ: проект — поле задачи, а с 05.09.2026 и желания.
     *
     * Замыслу, сведению и чувству признак не значит ничего: подробности
     * и причина правки — в `canBeProject`.
     */
    let isProject = item.isProject;
    if (isProject && !canBeProject(type)) {
      isProject = false;
      corrections.project++;
    }
    ...
    if (isActionable(type)) {
      ...
      if (resolvedRecurrence.problem !== undefined) {
        ...
        /**
         * Строка журнала называет, что осталось у записи (задача 3.76).
         *
         * Прежде она говорила «сохраняю фразой» во **всех** случаях — и
         * в том, где фразы нет вовсе. Именно так и вышло на боевом: восемь
         * предупреждений за трое суток, все с причиной «вид повторения без
         * фразы человека», и по строке выходило, будто фраза у записей
         * сохранена. По базе — ни одной: у этих записей не осталось
         * ничего, и это правильно (модель назвала повторение, которого
         * человек не говорил), но узнать это по журналу было нельзя.
         *
         * Урок тот же, что с процентом без строки: сообщение, которое
         * говорит неправду в половине случаев, стоит часа при следующем
         * разборе.
         */
        logger?.warn(
          { promptVersion, reason: resolvedRecurrence.problem },
          resolvedRecurrence.text === undefined
            ? 'Повторение отклонено целиком: фразы человека под ним нет'
            : 'Правило повторения не собралось — у записи остаётся фраза человека',
        );
      }
    ...
    /**
     * У дела со сроком важности «никакая» не бывает (задача 3.57).
     *
     * **Найдено живым прогоном 04.09.2026.** «Помыть машину в пятницу»
     * получило тип `TASK`, срок на сегодня — и важность `NONE`. В списке
     * «на сегодня» дела не было вовсе: выдача отсекает `NONE`, и
     * отсекает правильно — §6.3 держит так желания и эмоции вне выдачи
     * (`isShowable` в `output/filter.ts`). Человек назвал день и не
     * увидел дела в этот день.
     *
     * **Это не спор с моделью, а несогласованность её ответа**, ровно как
     * с точностью срока у регулярного дела выше. Важность `NONE` означает
     * «в выдачу не попадает». Дело, которому человек назвал срок, в
     * выдачу попасть обязано, иначе срок не значит ничего. Два поля
     * ответа противоречат друг другу, и код приводит их в согласие.
     *
     * Поднимается до `SOON`, а не до `NOW`: `NOW` — это «прямо сейчас», и
     * ставить его за человека значило бы двигать дело вперёд чужих.
     * Место в сегодняшнем списке даёт сам срок, а не важность.
     */
    const withUrgency =
      withRule !== undefined && isActionable(type) && priority === 'NONE' ? 'SOON' : priority;

    if (withUrgency !== priority) {
      corrections.priority++;
      logger?.info(
        { promptVersion, важность: withUrgency },
        'У дела со сроком важность поднята: «никакая» означала бы, что срок ни на что не влияет',
      );
    }
