...
/**
 * Корень «будни» против своих соседей по списку (находка 10.09.2026).
 *
 * Соседние дни недели в `TIME_WORDS` усечены до корня — «сред»,
 * «пятниц», «суббот», «воскресен», «выходны», — и потому ловят любую
 * форму слова. «Будни» стоит **целым словом**, а поиск идёт подстрокой:
 * «будням» слова «будни» не содержит, и три падежа из шести проверка не
 * видит.
 *
 * Это не рассуждение, а разница в списке, поэтому здесь она записана
 * рядом: соседи ловят, «будни» — нет.
 */
describe('корень «будни» короче своих соседей по списку', () => {
  // Формы, где за «будн» случайно идёт та самая «и»: только их список
  // и ловит. «Будничный» сюда попадает, «буднями» — уже нет.
  const seen = ['будни', 'в будни', 'будничный день'];
  ...
  /**
   * Ровно те формы, которыми это и говорят: «по будням» — самая частая.
   * Она же стоит в контрольном наборе (`docs/eval/recurrence.json`).
   */
  const missed = ['по будням', 'в буднях', 'до будней', 'буднями'];

  for (const text of missed) {
    it(`НЕ видит времени в «${text}» — корень не усечён`, () => {
      expect(hasTimeWord(text)).toBe(false);
    });
  }

  // У соседей той же беды нет: у них в списке корень, а не слово.
  const neighbours = [
    'по понедельникам',
    'по вторникам',
    'по средам',
  ...
  for (const text of neighbours) {
    it(`сосед по списку словоформу ловит: «${text}»`, () => {
      expect(hasTimeWord(text)).toBe(true);
    });
  }
});
...
/**
 * Цена находки: «по будням» остаётся без срока — но **не всегда**.
 *
 * Первая дорога к сроку (`hasTimeWord` по словам человека об этом деле)
 * закрыта: слова о времени в них нет. Вторая — цитата модели — открыта
 * или закрыта в зависимости от того, **в какой форме** модель приведёт
 * цитату, и это не наше решение, а её.
 *
 * Поэтому здесь записаны все три исхода. Вместе они говорят главное:
 * поведение боевого разбора на этом деле зависит от формы цитаты, то
 * есть без прогона на живой модели неизвестно.
 */
describe('«по будням» без слова о времени: срок держится на форме цитаты', () => {
  /** Речь из контрольного набора, обрезанная до нужного предложения. */
  const SPEECH = 'По будням собирать ему обед в школу, а то опять всухомятку.';

  /** Слова человека об этом деле: своё предложение плюс текст единицы. */
  const SAID = 'По будням собирать ему обед в школу собирать сыну обед в школу';

  it('слов о времени в словах человека нет', () => {
    expect(hasTimeWord(SAID)).toBe(false);
  });

  it('цитаты нет — срок отбрасывается', () => {
    const outcome = resolveDeadline(
      { deadline: '2026-08-28', accuracy: 'day' },
      { now: NOW, timeZone: ZONE, said: SAID, quoted: '', spoken: SPEECH },
    );
  ...
  it('модель процитировала «будни» — цитаты нет в речи дословно, срок отбрасывается', () => {
    // Именительный падеж слово о времени содержит, но в речи стоит
    // «будням»: условие дословности не выполняется.
    const outcome = resolveDeadline(
      { deadline: '2026-08-28', accuracy: 'day' },
      { now: NOW, timeZone: ZONE, said: SAID, quoted: 'будни', spoken: SPEECH },
    );
  ...
  it('модель процитировала «по будням» — срок проходит, но по случайности', () => {
    /**
     * **Это не работающая проверка, а совпадение букв.** Цитата обязана
     * назвать время; `hasTimeWord` в ней времени не видит — как и в
     * словах человека. Проходит она через список слов о часе
     * (`CLOCK_WORDS`), где стоит «дня»: «бу**дня**м» его содержит.
     *
     * Значит срок у этого дела сегодня держится на том, что модель
     * привела цитату в дательном падеже. Приведи она «будни» или ничего
     * — срока не будет (два теста выше).
     */
    const outcome = resolveDeadline(
      { deadline: '2026-08-28', accuracy: 'day' },
      { now: NOW, timeZone: ZONE, said: SAID, quoted: 'по будням', spoken: SPEECH },
    );

    expect(outcome.ok).toBe(true);
  });

  it('«дня» внутри «будням» — то самое совпадение, названное прямо', () => {
    // Цитата признана без единого слова о времени по мерке hasTimeWord.
    expect(hasTimeWord('по будням')).toBe(false);
    expect(timeQuoteInSpeech('по будням', SPEECH)).toBe(true);

    // Убери «дня» из слова — и цитата больше не признаётся. Тем самым
    // названа именно та буквенная случайность, а не что-то ещё.
    expect(timeQuoteInSpeech('по буднм', 'по буднм собирать обед')).toBe(false);
  });
});
