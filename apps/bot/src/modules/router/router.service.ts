...
export async function routeIntents(deps: AiClientDeps, params: RouteParams): Promise<RouteResult> {
   ...
   * разбор — запись появится, потерян будет один вызов модели. Так
   * бывает, пока в выгрузке до него не прозвучала мысль; сегмент **после
   * мысли** разбирается уже после сохранения, и вернуть его в разбор
   * нечем — он ложится черновиком, а человеку об этом сказано
   * («Сохранила целиком»). Ревизия этапа 3, A4-средняя: раньше здесь
   * обещалось «запись всё равно появится» без этой оговорки. Поэтому
   * признак требует двух примет сразу, а не одной.
   */
