 ...
 */
export const OPEN_STATUSES = ['new', 'active', 'in_progress', 'waiting', 'snoozed'] as const;

/**
 * `snoozed` — открытый статус, и это найдено ревизией этапа 3 (C1).
 *
 * Кнопка «Отложить» обещает «Напомню позже», а без `snoozed` в этом
 * списке дело исчезало отовсюду навсегда: из «Сегодня», из списков сферы,
 * из планировщика — напоминание по новому сроку не ставилось никем, и
 * назад статус не поднимал никто. Отложенное дело — не закрытое: у него
 * есть срок, к которому оно возвращается. Единственное, где его до этого
 * срока не должно быть, — выдача; это правило живёт в `isShowable`, а не
 * здесь, потому что зависит от «сейчас».
 */
