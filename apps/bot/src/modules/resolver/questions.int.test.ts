...
beforeEach(async () => {
  ...
  /**
   * Выгрузка уже разобрана.
   *
   * Так и бывает у вопроса, который ждёт человека часами: разбор длится
   * секунды и кончается задолго до ответа. Статус здесь важен: уборка
   * протухших нарочно не трогает вопрос выгрузки, которую разбирают
   * прямо сейчас, — иначе ответ человека оказался бы отвечать нечему.
   */
...
describe('три пути, которыми вопрос кончается', () => {
  ...
  it('вопрос идущего разбора уборка не трогает', async () => {
     ...
     *
     * Разбор читает открытый вопрос дважды — в начале, чтобы сказать о
     * нём модели, и в конце, применяя ответ, — а между чтениями лежит
     * вызов модели, то есть секунды. Закрой строку в этот промежуток, и
     * ответ человека «да, к прошлой» окажется отвечать нечему: правка не
     * применится, слова уйдут в никуда, а человеку скажут «Остальное из
     * этой фразы сохранила отдельно» — что читается как «основное
     * записала».
     *
     * Гонка появилась вместе с самой уборкой 10.09.2026: до неё срок
    ...
    expect(
      ...
      'уборка закрыла вопрос под идущим разбором',
    ).toBe(0);
  });
