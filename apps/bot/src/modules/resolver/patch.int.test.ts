...
describe('просьба запомнить у существующего дела (задача 3.8б)', () => {
  ...
  it('якорь правила берётся в поясе человека, а не в UTC', async () => {
    /**
     * **Задача 3.74.** Схема правила говорит прямо: «`anchor` — дата
     * первого повторения **в поясе человека**». А брался он из
     * `toISOString()`, то есть по UTC.
     *
     * Срок хранится мгновением: четверг 03.09 у москвича — это 02.09
     * 21:00 по UTC. Значит якорем становилась **среда**, и правило
     * «каждый четверг» превращалось в «каждую среду» — навсегда и у
     * каждого, кто восточнее Гринвича, то есть у всех наших: в Москве
     * промах ловится в последние три часа суток, в Омске — в шесть.
     *
     * Тому же промаху здесь подыграл сам тест: `THURSDAY` — четверг у
     * москвича, и `toISOString()` от него даёт 02.09, среду.
     */
    const { item } = await setRule('запомни, каждый четверг', {
      ...RULE,
      recurrenceKind: 'weekly',
      recurrenceText: 'каждый четверг',
    });

    const rule = (await reread(item.id)).recurrenceRule as { anchor: string };
    expect(rule.anchor).toBe('2026-09-03');
  });

  it('якорь берётся из исправленного срока, а не из строки модели', async () => {
    /**
     * Ревизия этапов 1–2: тот же дефект, что в классификации, — здесь.
     *
     * Правило строилось **до** разбора срока и брало строку модели. А
     * строку тут же ниже пересчитывает `nearestWeekday`: человек сказал
     * «в четверг», модель ответила средой, срок исправляется на четверг —
     * и якорь оставался средой. «Каждый четверг» становилось «каждой
     * средой» навсегда, а в карточке стояли слова человека про четверг.
     *
     * Починка — порядок: сперва срок, потом правило.
     */
    ...
    // Ближайший четверг от субботы 29 августа — 3 сентября: срок
    // исправлен со среды модели на него.
    expect(after.deadlineAt?.toISOString()).toBe('2026-09-02T21:00:00.000Z');

    // …и якорь обязан быть тем же четвергом, а не средой модели.
    expect(rule.anchor, 'правило разошлось со сроком той же записи').toBe('2026-09-03');
  });
