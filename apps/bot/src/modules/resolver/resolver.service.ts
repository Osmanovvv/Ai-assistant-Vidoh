 ...
 * решать не из чего. Вызов стоил бы денег и не мог бы изменить исход.
 */
...
const CREATE: Decision = {
  ...
  action: 'new',
  /**
   * Кандидатов не было — но это не значит «новая мысль».
   *
   * Соблазнительно решить, что раз править нечего, значит сказанное
   * самостоятельно. Тогда «нет, в пятницу» у человека без записей
   * станет задачей «в пятницу». Второй этап этот выбор уже сделал в
   * пользу черновика, и менять его молча нельзя: цена ошибки прежняя —
   * бессмыслица в списке дел, которую человек не просил.
   */
  newThought: false,
...
export async function resolveSegment(
  ...
  if (!outcome.ok) {
    ...
    return {
      ok: false,
      // Модель молчит — значит про «новая это мысль или правка» мы не
      // знаем ничего. Записью сказанное не станет: сохранится черновиком.
      decision: { ...CREATE, newThought: false, why: 'модель не ответила' },
