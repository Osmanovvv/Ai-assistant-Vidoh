  ...
  },
  {
    ...
    decision: {
      ...
      internal: [
        ...
        'awaitingInput',
        // Когда нажали «напишу своё» — момент того же ожидания, от
        // которого считается его четверть часа (дефект 28 ревизии 1–2).
        // Служебная бухгалтерия диалога, а не факт о человеке.
        'awaitingSince',
        'updatedAt',
      ],
    },
  ...
  },
  {
    ...
    decision: {
      exported: [
        ...
        'backgroundedAt',
        // Запрос №4: «Позже» — решение человека о своём деле, отдаётся.
        'deferredAt',
      ...
      internal: [
        ...
        'embedding',
        // Запрос №4: служебные отметки утреннего разбора — когда дело
        // показано и когда предложено из «Позже». О человеке не говорят
        // ничего сверх самого дела; его решение — `deferredAt` — отдаётся.
        'offeredAt',
        'reviewedAt',
      ],
    },
  ...
  },
  {
    ...
    decision: {
      ...
      internal: [
        ...
        'audioDurationSec',
        'batchId',
        // Служебная отметка «съедено как ответ на вопрос»: о человеке не
        // говорит ничего сверх самого текста, который выгружается.
        'consumedAt',
        // Та же природа: «без выгрузки нарочно, вот почему» — отметка
        // приёма, а не сведение о человеке.
        'refusedReason',
      ],
    },
  },
...
const NOT_PERSONAL: readonly string[] = [
  ...
  'billing_invoices',
  /**
   * Согласия на автосписания (§14, оферта п. 7.2.2).
   *
   * Тот же случай, что счета: тариф, сумма, адрес оферты, время нажатия
   * — и ни строчки сказанного человеком. Это наша обязанность перед
   * платёжной системой («сохраняйте историю согласий»), а не данные
   * человека о себе: само нажатие у него в переписке с ботом. При
   * удалении **обезличивается** (`set null`), как счёт, — строка без
   * человека не говорит ни о ком, а история согласий обязана пережить
   * удаление. Запросить свои согласия человек может через /paysupport,
   * как обещает оферта.
   */
  'billing_consents',
  /**
