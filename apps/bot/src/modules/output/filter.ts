 ...
 *
 * Уровень сил сюда не входит — и по устройству, и по подписи (ревизия
...
export function selectForToday(
   ...
   */
  /**
   * Просроченного здесь больше нет (запрос на изменение №4, решение
   * заказчицы 13.09.2026; ревизия этапа 3, E13). Оно не висит первым день
   * за днём: разбирается один раз утром — «На сегодня», «Позже» или
   * «Убрать», — а нетронутое само уходит в «Позже». В разборе выгрузки
   * (`selectForOutput`) просроченное остаётся: там оно живёт не дольше
   * суток, до ближайшего разбора вчерашнего.
   */
  const wanted = new Set<Bucket>([BUCKET.today, BUCKET.now]);
