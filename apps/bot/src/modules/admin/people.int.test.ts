...
describe('сказанное вне выгрузок: намеренное и случайное', () => {
  /**
   * Ревизия этапов 1–2, молчаливый отказ.
   *
   * Список «без выгрузки» подписан «так бывает после отказа гейта» — то
   * есть обещает потери. А лежали в нём вперемешку команды и служебные
   * сообщения, которые остаются без выгрузки **нарочно**: иначе бот
   * ответит «Слушаю.» на `/delete_my_data` и потом зачитает её обратно
   * расшифровкой. Намеренное и случайное были неразличимы, значит список
   * читался как факт, которым не был: разбирающий жалобу видел десяток
   * строк там, где потеряна одна.
   */

  it('команда в список потерь не попадает, а настоящая потеря попадает', async () => {
    await testDb()
      .insert(messagesRaw)
      .values([
        {
    ...
    expect(card?.orphans.map((row) => row.text)).toEqual(['записать сына к врачу']);
    // Итог считается тем же условием, что и список: разойдись они — и
    // подпись «показаны последние N из M» начнёт врать.
    expect(card?.orphansTotal).toBe(1);
  });
});
