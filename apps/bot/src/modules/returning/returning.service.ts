  ...
  const moved = await db
    ...
    .where(
      and(
        ...
        /**
         * Регулярные остаются (решение заказчицы 13.09.2026, ответ 1.5;
         * ревизия этапа 3, H2). «Каждый вторник плавание» — просьба
         * помнить, и пауза её не отменяет: человек не передумал, он
         * отсутствовал. Выключается регулярное только отдельно, по его
         * слову.
         */
        isNull(items.recurrenceRule),
      ),
    )
