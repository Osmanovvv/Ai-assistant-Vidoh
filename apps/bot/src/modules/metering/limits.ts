 ...
 * 30% от цены месячной подписки; сумма задаётся переменной окружения.
 * Обещание отдать её в панель стояло здесь вторым экземпляром и снято
 * ревизией панели вместе с первым — разбор в `config/env.ts`, там же
 * причина: лимит держат выключенным, пока нет настоящего распределения
 * расхода, а ручка от выключенного заслона — настройка без последствий.
 *
