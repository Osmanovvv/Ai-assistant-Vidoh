...
describe('вызов без цены', () => {
  ...
  it('часть без цены — сумма названа нижней границей, а не итогом', async () => {
    /**
     * Смешанное множество — самый частый случай, и до этой проверки его
     * не было ни здесь, ни на стенде. Найден на стенде после первой
     * починки оговорки: у отказа 429 цена ноль (он не тарифится, и ноль
     * здесь правда), у сорвавшегося по таймауту цены нет вовсе.
     *
     * Панель выбирала слова по «сумма пуста» — то есть по случаю «цены
     * неизвестны у всех» — и печатала «Ещё 0.00 ₽ потрачено вне
     * выгрузок — 2 обращения». Читающий делал вывод «оба обошлись в
     * ноль», хотя второй не оценён.
     *
     * Поэтому число «сколько из них без цены» обязано доехать до панели
     * отдельно от суммы: по нему она и различает точную сумму от нижней
     * границы.
     */
    await call({ rubles: 4 });
    await call({ model: 'yandex:новая-модель/latest' });
    await call({ ok: false });

    const report = await costBreakdown(testDb(), { since: SINCE });

    // Все три вне выгрузок и все три обезличены: ни человека, ни выгрузки.
    expect(report.unlinkedCalls).toBe(3);
    expect(report.unattributedCalls).toBe(3);

    // Без цены — только удавшийся: у сорвавшегося её и быть не может.
    expect(report.unlinkedUnpriced).toBe(1);
    expect(report.unattributedUnpriced).toBe(1);

    // Сумма при этом есть — и именно поэтому её нельзя звать итогом.
    expect(rublesOf(report.unlinked)).toBe(4);
    expect(report.unlinked).not.toEqual([]);

    // И отчёт объявлен неполным: пробел в прайс-листе настоящий.
    expect(report.complete).toBe(false);
  });

  it('все сорвались — «ничего» это факт, а не пробел в прайс-листе', async () => {
    /**
     * Обратная сторона предыдущей проверки, и она про доверие к отчёту.
     * У отказа цены нет и быть не может, поэтому сумма пуста — но
     * «неизвестно» тут неправда: неизвестного нет, есть ноль. Скажи
     * иначе — и заказчица пойдёт искать пробел в прайс-листе, которого
     * не существует.
     */
    await call({ ok: false });
    await call({ ok: false });
    ...
    expect(report.unlinkedCalls).toBe(2);
    expect(report.unlinkedUnpriced).toBe(0);
  ...
  it('а сорвавшийся вызов отчёт неполным НЕ делает: у отказа цены и не бывает', async () => {
     ...
     * вызовов цена неизвестна — модели нет в прайс-листе». Утверждение
     * конкретное, и врать им нельзя.
     *
     * Прежде число считалось как «цены нет» без условия на успех, а у
     * отказа цены нет и быть не может: 403, таймаут и обрыв не
     * тарифицируются, `callCost` на пустом ответе возвращает `null`.
     * Значит один сбой модели за период — и заказчица читает «расход
     * выше показанного, модели нет в прайс-листе», хотя прайс-лист
     * полон. Оговорка, которая горит после любого сбоя, перестаёт
     * значить что-либо, а настоящий пробел прайса от неё не отличить.
     *
     * Сорвавшийся вызов при этом не теряется: он виден своим числом
     * `failed` — это утверждается ниже, чтобы починка не превратилась в
     * «спрятали неудобное».
     */
    await call({ rubles: 2 });
    await call({ stage: 'router', ok: false });
