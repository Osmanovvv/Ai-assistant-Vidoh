...
function totalOf(rows: readonly CostRow[]): Money[] {
  ...
  /**
   * Порядок валют задан нами и здесь тоже (ревизия панели).
   *
   * Внутри строки разреза валюты сортируются (см. `fold`), а итоги
   * собирались `Map` в порядке обхода строк — то есть в том, какой
   * выбрал план запроса. Задевало это «На человека»: он один
   * складывает **много** строк и наследовал порядок валют первой из
   * них, тогда как «На выгрузку» приходит из фолда с одним ключом уже
   * отсортированным. Два соседних итога на одном экране могли показать
   * «₽, $» и «$, ₽» — тот же дефект, за который уже платили внутри
   * строки разреза и в расходе человека, где он ронял проверку через
   * раз.
   *
   * Сортировка по имени валюты, а не по столбцу: `currency` —
   * перечисление Postgres, объявленное как `('usd', 'rub')`, и порядок
   * по нему шёл бы по порядку объявления, а не по алфавиту.
   */
