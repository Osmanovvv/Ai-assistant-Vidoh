...
export const PRICING: Readonly<Record<string, ModelPricing>> = {
  /**
   * Воспроизведение записи стоит ноль, и это записано ценой, а не
   * пропуском (задача 3.80).
   *
   * Без этих двух строк воспроизведённые вызовы попадали бы в учёт как
   * «цена неизвестна» — и отчёт о расходе каждый раз говорил бы «это
   * нижняя оценка». Предупреждение, которое горит всегда, не значит
   * ничего; а настоящий случай — модели нет в прайсе — от него было бы
   * не отличить.
   *
   * Ноль здесь — правда: сеть не тронута, денег не ушло.
   */
  'cassette:llm': { kind: 'tokens', currency: 'rub', inputPerMillion: 0, outputPerMillion: 0 },
  'cassette:embedder': {
    kind: 'tokens',
    currency: 'rub',
    inputPerMillion: 0,
