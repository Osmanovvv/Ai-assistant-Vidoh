...
const expectedUnitSchema = z.object({
  ...
  /**
   * Большая цель или обычное дело (§5, задача 3.68).
   *
   * **Набор этого не мерил вовсе**, и живой прогон проджекта 04.09.2026
   * показал, чего это стоило: «хочу начать делать небольшой сайт» стало
   * **желанием**, а его же шаг «определиться со структурой сайта» —
   * **проектом**. Роли перевёрнуты, шагов в базе ноль, и на вопрос «какой
   * следующий шаг?» отвечать было нечем. Пока признак не мерится, любую
   * починку здесь нельзя ни доказать, ни защитить.
   *
   * По умолчанию звёздочка — «разметка не решает». Прежние случаи от
   * появления поля не краснеют: проекты в них не размечены, и требовать
   * их задним числом значило бы мерить не продукт, а свою невнимательность.
   */
  isProject: z.union([z.boolean(), z.literal('*')]).default('*'),
  /**
 ...
 *
 * **Зачем понадобилось.** В речи человек передумывает вслух: «Ещё в
 * четверг хотел заехать я на мойку. Хотя нет, давай мойку лучше в
 * пятницу». Отменённый замысел записью стать не должен — иначе человек
 * получит в списке дело, от которого сам же отказался вслух, и перестанет
 * верить остальным записям.
 *
 * **Почему отдельным числом, а не через «лишних».** Лишние записи
 * считаются долей с порогом 25%: там живёт законное дробление, и одна
 * лишняя запись порог не валит. Отменённое — не шум дробления, а прямое
 * противоречие сказанному, и порог по нему ноль, как у выдуманных сроков.
 *
 * **Проверяется только по незанятым записям.** Запись, закрывшая
 * обязательное ожидание, — законная: если разбор написал «помыть машину
 * на мойке в пятницу», это верное дело, а не отменённое. Нарушение — это
 * **добавочная** запись, несущая отменённый замысел.
 */
const retractedPlanSchema = z.object({
  /** Корни, по которым узнаётся отменённый замысел. Все обязательны. */
  keywords: z.array(z.string().min(2)).min(1),
  /** Чем именно человек это отменил. Идёт в отчёт рядом с нарушением. */
  why: z.string().default(''),
});

export type RetractedPlan = z.infer<typeof retractedPlanSchema>;
...
export const evalCaseSchema = z.object({
  ...
  expected: z.object({
    ...
    /** Замыслы, от которых человек отказался вслух. Порог по ним ноль. */
    retracted: z.array(retractedPlanSchema).default([]),
  }),
});
