...
const expectedUnitSchema = z.object({
  ...
  deadlineDate: z
    ...
    .optional(),
  /**
   * Двоякое дробление: разбор вправе сделать эту единицу, а вправе и не
   * делать (задача 3.53).
   *
   * **Зачем понадобилось.** В речи есть места, где одна мысль законно
   * становится и одной записью, и двумя. «Еще взять номер телефона.
   * Домика в Волконке и узнать о наличии свободных мест может быть
   * запланировать поездку на 5 7 сентября» — точка посреди мысли; разбор
   * то склеивает это в одну запись, то делит на две, и оба чтения верны.
   * То же с «может быть, купить туда стеллаж, пуфики… может быть, там
   * кальянную зону сделать».
   *
   * Набор такие места **наказывал дважды**: склеил — одно ожидание
   * осталось непокрытым и пошло в потери; разделил — вторая запись пошла
   * в лишние. Ни то ни другое дефектом не было, а числа портились, и на
   * них потом принимались решения о промптах.
   *
   * **Помечать, а не подгонять.** Необязательное ожидание не считается
   * ни потерей, ни лишней записью, и **из знаменателя тоже исключается**:
   * иначе доля найденного зависела бы от того, склеил разбор или нет.
   * Ставить эту метку можно только там, где **оба** чтения верны, и
   * причина обязана быть написана в `why`. Место, где верно ровно одно
   * чтение, остаётся обычным ожиданием — иначе метка превратится в
   * способ прятать дефекты.
   */
