 ...
 * вторую выгрузку.
 *
 * Сдвиг 900000000 — соглашение своих баз (`vydoh_e2e`, тестовая): кроме
 * тестов апдейтов там нет. На боевой базе номера выдаёт Telegram, и
 * «отведённых под тесты» диапазонов там не бывает — поэтому сквозной
 * тест этапа 1 (`ops/e2e-stage1.sh`) ведёт номера сообщений от времени
 * запуска, куда счётчик Telegram не дорастёт, и журнал апдейтов боевого
 * бота не чистит.
 */
