...
function nextFridayInMoscow(): string {
  ...
  /**
   * **Включая сегодня** — то же правило, что у продукта.
   *
   * `nearestWeekday` в разборе дат говорит прямо: «„в четверг",
   * сказанное в четверг, — это сегодня, а не через неделю: человек
   * говорит о ближайшем, иначе он сказал бы „в следующий"». Помощник
   * шагал строго вперёд и с продуктом расходился — но только по пятницам,
   * то есть один день из семи. Шесть дней в неделю он совпадал случайно,
   * и найти это можно было лишь тем, что однажды наступила пятница
   * (11.09.2026).
   *
   * Второй способ посчитать одно и то же число — ровно та ошибка, за
   * которую проект уже платил; здесь он остаётся, потому что проверка
   * обязана считать независимо, но правило теперь одно и то же.
   */
