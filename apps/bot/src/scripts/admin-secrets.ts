...
process.stdout.write(
  [
    ...
    `ADMIN_LOGIN=${login}`,
    /**
     * Хэш — в одинарных кавычках. В нём есть `$`, а Docker Compose
     * подставляет переменные и в `env_file`: `$abc` без переменной `abc`
     * превращается в пустоту, и хэш доезжает до бота укороченным. Так и
     * вышло на бою 12.09.2026: 84 знака в файле, 61 в контейнере, вход
     * «не получилось» на верном пароле. Кавычки Compose снимает, а
     * внутри них ничего не подставляет.
     */
