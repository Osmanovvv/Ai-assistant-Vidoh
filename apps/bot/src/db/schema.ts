...
export const userSettings = pgTable('user_settings', {
   ...
   *
   * Пусто почти всегда, и это главное свойство колонки: пока здесь `null`,
   * путь каждого входящего сообщения ровно такой, каким был до задачи.
   * Значение появляется **только** после явного нажатия «напишу своё» —
   * то есть человек сам сказал, что следующая его реплика будет ответом,
   * а не мыслью.
   *
   * Почему это вообще понадобилось. Онбординг был целиком на кнопках, и в
   * `onboarding.service.ts` написано почему: свободный ответ приходит
   * обычным сообщением и попадает в буфер выгрузки, а угадывать «ответ это
   * или новая мысль» значит однажды угадать неверно и потерять мысль.
   * Явное нажатие снимает догадку: развилки больше нет, человек назвал
   * её сам. А если присланное не похоже на ответ — оно уходит в разбор,
   * как и раньше, и ожидание снимается.
   *
   * Значения: `name`, `morning`, `evening`, `edit:<id записи>`.
   */
  awaitingInput: text('awaiting_input'),

  /**
   * Имя, которое человек выбрал себе сам (задача 3.61).
   *
   ...
   * репликой человека.
   *
   * Пусто — значит человек своего имени не называл, и берётся то, что дал
   * Telegram, как было до задачи.
   */
...
export const batches = pgTable(
  'batches',
  {
    ...
    /**
     * Статусное сообщение занято ответом по существу (§9.2).
     *
     * Пока в нём «Секунду, слушаю запись», править его безопасно. Как
     * только туда лёг ответ — с кнопкой «Отменить», например, — следующая
     * правка сотрёт и текст, и кнопку. Обработчик выгрузки держал этот
     * признак в памяти, а докладчик о сбое живёт вне его и потому затирал
     * подтверждение правки; на повторе оно не возвращалось, потому что
     * вопрос уже закрыт. Ставится там, где слот и занимают, — в
     * `showStatus` при финальной правке.
     */
    statusTaken: boolean('status_taken').notNull().default(false),
    ...
    /**
     * Момент, когда этой выгрузкой пробный период **исчерпан** (4.4).
     *
     * Один на человека, и это гарантия базы. Отличается от
     * `trialCountedAt` тем же, чем «последняя капля» от «капли»: та
     * ставится каждой выгрузке, съевшей период, эта — только той, после
     * которой бесплатного не осталось.
     *
     * Нужен затем, что предел правится из панели без выкладки, а истории
     * у настроек нет: посчитай мы «конец пробного» из нынешнего предела —
     * и каждая правка сдвигала бы третий шаг воронки у всех сразу, задним
     * числом. Отчёт о запуске у блогера менялся бы от настройки,
     * сделанной через месяц.
     *
     * Своей таблицей событий это не сделано нарочно: строка события
     * соответствовала бы один к одному строке выгрузки, то есть стала бы
     * второй правдой о ней.
     */
    trialOverAt: timestamp('trial_over_at', { withTimezone: true }),

    /**
     * Предел, действовавший в ту секунду.
     *
     * Без него момент нем: «пробный кончился» без числа не говорит, на
     * чём именно он кончился. Ставится вместе с моментом и только с ним —
     * за этим следит ограничение базы.
     */
    trialLimit: integer('trial_limit'),
  ...
  (table) => [
    ...
    /**
     * Один момент «конец пробного» на человека — гарантией базы.
     *
     * Иначе повторный проход или второй процесс записали бы второй
     * конец, и третий шаг воронки посчитал бы человека дважды.
     */
    uniqueIndex('batches_trial_over_once_idx')
      .on(table.userId)
      .where(sql`${table.trialOverAt} is not null`),

    // По нему воронка отбирает моменты за окно когорты.
    index('batches_trial_over_idx')
      .on(table.trialOverAt)
      .where(sql`${table.trialOverAt} is not null`),
  ],
);
...
export const items = pgTable(
  'items',
  {
    ...
    /**
     * Запись ушла в фон (§13.6 ТЗ, «начать с чистого листа»).
     *
     * Не удаление и не отмена: человек не передумал делать это дело — он
     * решил не держать старое перед глазами. §13.6 прямо требует, чтобы
     * такие записи остались доступны через бэклог.
     *
     * Отдельная колонка, а не статус `snoozed`: тот уже занят кнопкой
     * «Отложить» у карточки и заодно двигает срок. Смешав их, мы получили
     * бы «начать с чистого листа», переносящее сроки, — и никто бы не
     * понял почему.
     */
    backgroundedAt: timestamp('backgrounded_at', { withTimezone: true }),
...
export const pendingQuestions = pgTable(
  'pending_questions',
  {
     ...
     *
     * **Этого поля не было, и вопрос про дополнение применялся как
     * замена.** «А ещё туда надо взять карту прививок» при двух похожих
     * записях даёт вопрос; человек нажимает «Добавить к прошлой», бот
     * отвечает «Добавила к прошлой» — а подробность не появляется
     * нигде: применение по умолчанию заменяет, менять оказывается
     * нечем, и note выбрасывается. Слова человека пропадали, а реплика
     * при этом врала (§9.1 и §7.4 разом).
     *
     * Пусто означает «замена»: так вело себя применение до этой правки,
     * и прежние открытые вопросы должны дожить с прежним поведением.
     */
    mode: text('mode'),
...
export const reminders = pgTable(
  'reminders',
  {
    ...
    /**
     * Сколько раз отправка сорвалась (§5 ТЗ).
     *
     * Без счётчика неудачная отправка помечалась отправленной: сообщение
     * терялось молча, а человек, ничего не получивший, попадал в серию
     * молчания (3.17) — и продукт снижал ему частоту за собственный сбой.
     */
    attempts: integer('attempts').notNull().default(0),

    /**
     * Почему не отправили. Значения — в `SKIP_REASONS`
     * (modules/scheduler/reminders.repo.ts), там же их пояснения.
     *
     * Списком в коде, а не здесь: этот комментарий обещал 'quiet' и
     * 'rare', которых в базе нет ни одной строкой, и молчал про
     * 'blocked' и 'failed', которых большинство (задача 3.82).
     *
...
export const adminAccessLog = pgTable(
  'admin_access_log',
  {
    ...
    /**
     * Сколько человек попало в ответ. Пусто — **не установлено**.
     *
     * Прежде столбец был `not null default 1`, а записывать в него было
     * нечему: умолчание утверждало «в ответ попал один человек» про
     * страницу списка из двадцати. Пустая клетка читается как факт —
     * здесь она читалась как единица, и разбирающий инцидент сделал бы
     * из неё вывод, обратный правде.
     *
     * Число ставится там, где известно: у карточки — один, у списков —
     * после ответа, по числу выданных строк. Где не известно, стоит
     * пусто, и панель печатает это словом.
     */
    subjects: integer('subjects'),
 ...
 *
 * **Код называет цену, а не скидку.** Не процент и не бесплатность, и
 * оба отказа обоснованы кодом: `outSumOf` отвергает суммы меньше рубля,
 * звёздный счёт — меньше одной звезды, а ноль в тарифах означает «цена
 * не задана» и убирает кнопку оплаты. Счёт на нуль физически не
 * выставляется, значит «первый период бесплатно» — не разновидность
 * скидки, а отдельное устройство, требующее решения заказчицы.
 *
 * Процент отвергнут по другой причине: в панели заказчица называет цену,
 * а не долю. Фиксированная цена читается однозначно и не уползает при
 * смене цены тарифа.
 *
 * **Применение — это строка счёта, а не своя таблица.** Счёт заводится
 * на каждую попытку оплаты и уже несёт человека, рельс, тариф, сумму и
 * состояние. Отдельная таблица применений стала бы вторым источником
 * правды о том, кто заплатил по коду, и однажды разошлась бы: возврат
 * обрывает период, но строку счёта в «оплачено» не откатывает, а
 * удаление данных счёт обезличивает, а не удаляет.
 */
export const promoCodes = pgTable('promo_codes', {
   ...
   *
   * Обе обязательны. Паритет правил Telegram — про товар, а не про
   * скидки, но скидка только на карту подталкивала бы людей мимо звёзд,
   * то есть делала бы ровно то, против чего правило написано.
   */
  priceRubMinor: integer('price_rub_minor').notNull(),
   ...
   *
   * Считаются **оплаченные** счёта, а не выставленные: брошенный счёт
   * квоту тратить не должен, а счета без оплаты — нормальное состояние.
   */
  maxRedemptions: integer('max_redemptions'),

  /** Кому выдан: имя блогера, канал, договорённость. Для панели. */
  note: text('note'),
...
export const billingInvoices = pgTable(
  'billing_invoices',
  {
     ...
     *
     * Применение промокода — это и есть строка счёта: отдельной таблицы
     * применений нет нарочно, она стала бы вторым источником правды.
     */
    promoCode: text('promo_code').references(() => promoCodes.code, { onDelete: 'set null' }),

    /**
     * Цена без скидки — та, что была бы без кода.
     *
     * Выручка в обзоре складывает суммы оплаченных счетов; без полной
     * цены скидка блогерам выглядела бы провалом продаж, а посчитать
     * «сколько недополучено по кодам» было бы нечем.
     */
    amountFullMinor: integer('amount_full_minor'),

    /**
     * Обещал ли провайдер автопродление по этому счёту.
     *
     * Записывается ответом провайдера при выставлении счёта — он один
     * знает правду: у звёзд годовой тариф продлеваться не умеет вовсе, а
     * у Робокассы продление работает только после согласования услуги.
     * Догадка «месячный — значит продлевается» неверна на обоих рельсах.
     *
     * Пусто означает «не обещали», и направление отказа выбрано так
     * нарочно: не продлить обещанное дешевле, чем списать необещанное.
     */
     ...
     *
     * Ключ запрета второго списания за тот же период. Дочернее списание
     * берёт деньги без участия человека, и второе такое — не лишняя
     * запись, а дважды снятые деньги. Замок в Redis этого не удержит:
     * перезапуск в середине прохода и потерянный ответ Робокассы
     * («не ответила» ≠ «не списала») ломают его оба.
     *
     * Пусто у первых платежей: им продлевать нечего.
     */
    renewsPeriodEnd: timestamp('renews_period_end', { withTimezone: true }),
    ...
    /**
     * Когда счёт стал неудачным (ревизия панели).
     *
     * У оплаты и возврата свои даты, а у отказа не было никакой, и
     * журнал сбоев отбирал неудачные платежи по `createdAt` — по дате
     * счёта. Но счёт заведён в момент нажатия кнопки и живёт до
     * `expiresAt`, а недоплата приходит уведомлением тогда, когда
     * человек соберётся заплатить: счёт трёхдневной давности,
     * недоплаченный сегодня, при выборе «сутки» в журнале не появлялся,
     * а в колонке «Когда» стояла дата счёта — не время события,
     * которое разбирают.
     *
     * Пусто у счетов, помеченных неудачными до появления колонки:
     * задним числом время отказа неизвестно, и выдумывать его — значит
     * записать догадку. Журнал у таких берёт дату счёта, как и прежде.
     */
  ...
  (table) => [
    ...
    /**
     * Поиск счёта по нашей метке (ревизия четвёртого этапа).
     *
     * Комментарий в `pre_checkout_query` обещал «ровно одна проверка, по
     * индексу», а индекса не было ни одного: поиск шёл перебором. На
     * подтверждение звёздного платежа Telegram даёт **десять секунд**;
     * не ответили — платёж не состоится, и человек увидит отказ без
     * объяснений.
     */
    index('billing_invoices_ref_idx').on(table.provider, table.ref),
    /**
     * Одно списание на период — запретом базы, а не осторожностью кода.
     *
     * Частичный: у первых платежей `renewsPeriodEnd` пуст, и у них
     * никакой уникальности по паре «человек, рельс» быть не должно.
     */
