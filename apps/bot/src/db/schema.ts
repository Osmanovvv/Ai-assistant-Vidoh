...
export const users = pgTable(
  'users',
  {
     ...
     *
     * До 12.09.2026 ставился первым сообщением после экрана с политикой
     * (конклюдентно). С кнопкой «Согласна» (решение заказчицы 12.09.2026,
     * ответ 13) ставится нажатием; у зарегистрированных раньше здесь
     * остаётся момент прежнего согласия — история, а не подтверждение.
     */
    consentAt: timestamp('consent_at', { withTimezone: true }),
    /**
     * Нажатие «Согласна» — единственное, что считается согласием на
     * разбор (решение заказчицы 12.09.2026). Пусто — выгрузки не
     * разбираются, слова сохраняются и ждут нажатия.
     */
...
export const userSettings = pgTable('user_settings', {
  ...
  /**
   * Когда нажали «напишу своё» — от него считается окно ожидания.
   *
   * Окно в четверть часа было обещано как страховка от потери мысли:
   * нажал и отвлёкся на день — вернувшаяся мысль уйдёт в разбор. Отсчёт
   * шёл от `updatedAt`, а его двигают все правки строки: переключатели
   * меню, шаги опроса, время напоминаний. Каждая такая правка продлевала
   * окно, и мысль, сказанная через час после «Изменить», переписывала
   * запись (ревизия этапов 1–2, дефект 28). Момент нажатия — свой.
   *
   * Пусто при заполненном `awaitingInput` — нажали до миграции 0044:
   * момент неизвестен, окно обещать нечем, ожидание считается
   * просроченным. Задним числом не заполняется — по той же причине, что
   * `batches.processingAt`.
   */
...
export const messagesRaw = pgTable(
  'messages_raw',
  {
     ...
     * промокод) и в разбор не пошло — по замыслу (найдено на бою
     * 12.09.2026).
     *
     * Без этой отметки такая строка оставалась без выгрузки навсегда, и
     * счётчик сирот неделю писал «есть сообщения без выгрузки: 2» про
     * «7:30» из опроса. Тревога без причины приучает её не читать —
     * настоящая потеря утонула бы в шуме. Отметка, а не удаление: слова
     * человека остаются (§16).
     */
    consumedAt: timestamp('consumed_at', { withTimezone: true }),
    /**
     * Сообщение сохранено, а выгрузка по нему не заведена нарочно — и вот
     * почему: `trial` (пробный период кончился), `expired` (оплаченный
     * кончился), `renewalFailed` (продление не прошло) — §14; `dumpLimit`
     * — суточный потолок §10.5. Те же причины, что у гейта доступа.
     *
     * Без отметки такая строка через час становилась «сиротой» для
     * счётчика — та же болезнь, что у `consumed_at`, только с первым
     * тестером, упёршимся в лимит (13.09.2026). Слова остаются (§16).
     */
...
export const batches = pgTable(
  'batches',
  {
    ...
    /**
     * Когда выгрузку взяли в разбор.
     *
     * От неё, а не от закрытия, досмотр считает «застряла». Выгрузка,
     * пролежавшая в очереди час — так бывает, когда у нас кончился
     * доступ к модели и попытка нарочно не тратится, — по `closedAt`
     * выглядела застрявшей в первую же миллисекунду разбора: досмотр
     * возвращал живой разбор в очередь и писал «Подобрал выгрузки, о
     * которых очередь забыла».
     *
     * Пусто — взяли до этой правки; тогда порог считается от `closedAt`,
     * как раньше. Задним числом не заполняется: придумать за прошлое
     * время начала разбора нельзя.
     */
...
export const items = pgTable(
  'items',
  {
     ...
     * вчерашнего вместо хвоста просроченного.
     *
     * `deferred_at` — ушло в «Позже»: срок снят, из «Сегодня» и утреннего
     * убрано, дело осталось в «Все задачи» и в своей сфере. Кнопкой или
     * само, если в разборе ничего не нажали. Не статус и не фон
     * (`backgrounded_at`): в фон уходит «с чистого листа» всё старое, а
     * «Позже» — про одно дело и с возвращением: утром, когда дел мало,
     * одно из отложенного предлагается снова.
     * `offered_at` — когда его в последний раз предлагали утром: каждое
     * не чаще раза в неделю, по кругу от самого давнего.
     * `reviewed_at` — когда дело показано в разборе вчерашнего: показано
     * один раз, к следующему утру нетронутое уходит в «Позже».
     */
...
export const billingSubscriptions = pgTable(
  'billing_subscriptions',
  {
     ...
     * `telegram_payment_charge_id`. У Робокассы ключа нет и колонка пуста:
     * дочернее списание уходит от нас по этой же строке, а номер
     * материнского платежа для него берётся из счёта.
     */
