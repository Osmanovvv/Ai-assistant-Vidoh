 ...
 * 4. подписи кнопок на узком экране видны целиком.
 *
 ...
 *
 * **Четвёртый добавлен 01.09.2026, после ручного прогона.** В плане
 * стояло «три сценария, не больше», и три оказалось мало: третий мерил
 * ширину **текста** ответа и был зелён, пока прямо под этим текстом
 * кнопка показывала «Остави…потом» вместо «Оставить на потом». Проверка,
 * которая смотрит на половину сообщения, зелена ровно наполовину.
 *
...
test.describe('темы в Telegram Web', () => {
  ...
  test('подписи кнопок на узком экране видны целиком', async ({ page }) => {
    /**
     * Дефект, ради которого сценарий появился (01.09.2026).
     *
     * Три кнопки §13.2 стояли одной строкой. Telegram делит ширину строки
     * поровну, и на узком телефоне «Оставить на потом» показывалось как
     * «Остави…потом».
     *
     * **Многоточие стоит в середине, и это важно для проверки.** CSS так
     * не умеет — `text-overflow` режет только хвост. Значит подпись
     * укорачивает сам Telegram, ещё до отрисовки, и на странице просто
     * нет узла с полным текстом. Поэтому проверяется наличие подписи
     * целиком, а не геометрия: геометрия у обрезанной подписи как раз в
     * порядке — она для того и обрезана.
     */
    await page.setViewportSize({ width: 360, height: 740 });
    await openChat(page);
    ...
    const seen = await page.evaluate((labels: string[]) => {
       ...
       *
       * Без них тест говорит «подписи нет» и заставляет лезть за
