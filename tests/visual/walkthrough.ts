 ...
 * двухсот рублей.
 *
 * **Порядок шагов значит больше, чем кажется.** Уровень сил, названный
 * словами, живёт до конца суток человека (§13.7): «я на нуле» на шаге 6
 * урезает выдачу всех последующих шагов до одного дела. Поэтому проверки
 * числа дел идут **до** шага 6, а перед прогоном стоит убрать
 * `user_state` — иначе уровень достанется от прошлого прогона.
 *
 * Запуск:
...
try {
  ...
  // ── 5а. Усталость сокращает форму, но не список (задача 3.47) ───────
  //
  // **Обязан идти до шага 6.** «Я на нуле совсем» опускает уровень сил до
  // «пусто», и он живёт до конца суток человека — после шага 6 любая
  // выдача будет из одного дела, чем бы её ни просили.
  if (runs('5а')) {
    step('5а', 'Усталость: форма короткая, а дел всё равно три');

    const weary = await send(
      page,
      `Задолбался всё это в голове держать. Надо купить ${MARK} новый, позвонить в банк и забрать очки из оптики.`,
    );
    show(weary);

    const bullets = (weary.text.match(/^—/gmu) ?? []).length;
    check('в выдаче три дела, а не одно', bullets === 3, `строк списка: ${String(bullets)}`);
    check(
      'форма короткая: закрытие без вопроса',
      has(weary, defaultTexts.answer.closingTired) && questions(weary.text) === 0,
      weary.text,
    );
    check(
      'ответ не читает мораль',
      !has(weary, 'ты молодец') && !has(weary, 'соберись') && !has(weary, 'отдохни'),
      weary.text,
    );
  }
