...
test.describe('расходы (§15, §21 п.14; задача 4.7)', () => {
  ...
  test('сбой без цены не выдаётся за пробел в прайс-листе', async ({ page }) => {
    /**
     * Ревизия панели, мёртвый страж. Обстановки «сбой без цены» на стенде
     * не было вовсе: у единственного неуспешного вызова цена ноль (429 не
     * тарифится). Значит оговорку «модели нет в прайс-листе» не проверяло
     * ничто — убери условие на успех из `unpricedSql()`, и панель начнёт
     * печатать её после каждого таймаута модели, а весь отчёт объявлять
     * нижней границей. Оговорка, горящая от любого сбоя, не значит
     * ничего, и суммы рядом с ней перестают читаться как точные.
     *
     * Теперь стенд сеет ровно эту обстановку: сорвавшийся вызов без цены
     * и без человека — так выглядит расход того, кто удалил данные.
     */
    await signIn(page, 'Расходы');
    await expect(page.getByTestId('costs')).toBeVisible();

    await expect(page.getByText('модели нет в прайс-листе')).toHaveCount(0);

    /**
     * Сбой при этом не спрятан: он посчитан своей колонкой, а «Без цены»
     * у него пусто — у отказа цены не бывает.
     *
     * Колонки: 0 — ключ, 1 «Отправок», 2 «Сбоев», 3 «Без цены», 4
     * «Расход». Два сбоя против нуля без цены — это и есть разница,
     * которую снятое условие стирает.
     */
    const stages = page.locator('.разрез', { hasText: 'По этапам' });
    const row = stages.getByRole('row', { name: /classifier/u });
  ...
  test('обезличенный расход без цены сказан словами, а не прочерком', async ({ page }) => {
    /**
     * Прежде печаталось «Ещё — потрачено … 1 обращений»: прочерк вместо
     * суммы читался и как ноль, и как «неизвестно», а число не
     * согласовывалось со словом. Состояние штатное: §16 обнуляет
     * `user_id`, и у сорвавшегося вызова ушедшего человека нет ни
     * человека, ни цены разом.
     *
     * **И «неизвестно» здесь тоже неправда.** На стенде единственный
     * обезличенный вызов — сорвавшийся, а у отказа цены нет и быть не
     * может: он не тарифится. Значит «ничего» — факт, и назвать это
     * пробелом в прайс-листе значило бы позвать заказчицу искать
     * несуществующую дырку. Слова «расход по ним не записан» отведены
     * ровно под настоящий пробел: удавшийся вызов без цены.
     */
    await signIn(page, 'Расходы');

    const clause = page.getByTestId('unattributed');

    await expect(clause).toContainText('денег они не стоили');
    await expect(clause).toContainText('1 обращение');
    await expect(clause).not.toContainText('Ещё —');
    await expect(clause).not.toContainText('1 обращений');

    // Пробел в прайс-листе не объявляется там, где его нет.
    await expect(clause).not.toContainText('расход по ним не записан');

    // И «меньше общей ровно на эту величину» — тоже: величины нет.
    await expect(clause).not.toContainText('ровно на эту величину');
  });
