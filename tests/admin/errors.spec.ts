...
test.describe('журнал сбоев (§15; задача 4.10)', () => {
  test('показывает сорвавшийся разбор: у кого, на чём и сколько попыток', async ({ page }) => {
    ...
    /**
     * Число попыток — внутри своей клетки, а не «где-то на странице».
     *
     * Имя проверки его обещало, а проверялось только имя человека и
     * причина: убери колонку «Попыток» целиком — набор оставался
     * зелёным. Разбор «частично подтверждённых» поймал это на слове.
     *
     * Клетка третья по счёту: Когда, У кого, Попыток.
     */
  ...
  test('неуспешный вызов модели виден вместе с тем, платили ли за него', async ({ page }) => {
     ...
     *
     * **Проверка была мёртвой.** Она утверждала только текст причины и
     * версию промпта — обе клетки других колонок, — и на всей странице
     * сразу. Убери колонку «Заплатили» вместе с ячейкой — оставалась
     * зелёной. Теперь клетка сверяется внутри своей строки.
     *
     * **Почему «да» здесь не проверяется.** Платного сбоя стенд не
     * сеет, и заводить его сюда нельзя: цена сдвинула бы «Расход на
     * модели», который другие разделы читают глазами и точным текстом.
     * Случай «платили — да» закрыт на сервере, где обстановка ставится
     * без побочных чисел: `errors.int.test.ts`, «ровно один платный».
     */
    await signIn(page, 'Ошибки');

    const call = page
      .locator('[data-testid^="failed-call-"]')
      .filter({ hasText: '429 Too Many Requests' });

    await expect(call).toHaveCount(1);
    await expect(call).toContainText('classifier@9');

    /**
     * Клетка седьмая: Когда, У кого, Этап, Модель, Версия, Мс,
     * Заплатили. У 429 цена ноль — он не тарифится, и «нет» здесь
     * правда, а не пробел.
     */
    await expect(call.locator('td').nth(6)).toHaveText('нет');
  });
