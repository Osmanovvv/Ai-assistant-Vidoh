...
# запись отдельно (transcribeVoices в speech.service.ts). Три строки здесь —
# признак того, что склейка не сработала, а не того, что всё в порядке.
cat <<'EOF'
  ...
  В ai_calls — одна строка этапа speech на все три голосовых: они
  ушли одним склеенным запросом, и «секунд» у неё — три записи вместе
  плюс две секунды пауз между ними. Все строки успешные.
EOF
