...
# непривилегированным пользователем из образа (uid 1000). Без этой строки
# он не может создать в ней файл: журнал не пишется, а до починки логгера
# бот от этого ещё и падал в петлю перезапусков. Идемпотентно: `install`
# на существующей папке только выставляет права.
sshx "install -d -o 1000 -g 1000 -m 755 $REMOTE_DIR/logs"
