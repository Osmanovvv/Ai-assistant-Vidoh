...
# Причина — в выводе контейнера этого запуска, а не в файле журнала
# (ревизия этапов 1–2).
#
# Прежде хвост брался из logs/vydoh.log, а вывод контейнера — только если
# файл не прочитался. Файл переживает выкладку (задача 3.51), и для
# разбора вчерашнего это его достоинство, но здесь оно оборачивалось
# ложью: самый частый провал старта — отказ конфигурации — бросается до
# создания логгера и уходит в stderr контейнера, в файл не попадает ни
# строки, и выкладка показывала бы сорок бодрых строк прошлого,
# здорового прогона. Показывала бы: на деле перенаправления стояли в
# порядке «2>/dev/null >&2», и вывод tail уходил в /dev/null — под
# заголовком «последние строки журнала» не печаталось ничего.
#
# Логгер пишет в стандартный вывод и в файл одновременно, поэтому вывод
# контейнера содержит всё, что есть в файле за этот запуск, и сверх того
# то, чего в файле нет: отказ конфигурации, необработанное исключение,
# слова самого docker. Две строки, которые он показал 05.09.2026, были
# правдой о запуске: бот сказал две строки и ждал getMe, а прочитано
# было как «в docker logs ничего нет».
#
# Состояние контейнера — первой строкой: «restarting, перезапусков: 5»
# и «running, здоровье: starting, перезапусков: 0» — разные болезни, и
# по одному хвосту их не отличить. Список с -a: без него compose ps
# показывает только работающие, и контейнер, который завершился и не
# перезапускается, выглядел бы как «нет контейнера».
echo "не дождался готовности" >&2
container=\$($COMPOSE ps -a -q bot)
if [ -n "\$container" ]; then
  docker inspect --format 'контейнер: {{.State.Status}}, здоровье: {{.State.Health.Status}}, перезапусков: {{.RestartCount}}, код выхода: {{.State.ExitCode}}' \$container >&2 || true
else
  echo "контейнера bot нет" >&2
fi
echo "вывод контейнера с этого запуска (прошлые — в $REMOTE_DIR/logs/vydoh.log):" >&2
